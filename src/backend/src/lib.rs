...
//     match onnx::validate_id(image) {
//         Ok(result) => ClassificationResult::Ok(vec![Classification {
//             label: result.label,
//             score: result.score,
//         }]),
...
fn classify(image: Vec<u8>) -> ClassificationResult {
    ...
    let result = match onnx::validate_id(image) {
        Ok(result) => ClassificationResult::Ok(vec![Classification {
            label: result.label,
            score: result.score,
        }]),
    ...
    ic_cdk::println!("Executed instructions: {}", end - start);
    result
}
...
const IMAGE: &'static [u8] = include_bytes!("../assets/test00.jpeg");
